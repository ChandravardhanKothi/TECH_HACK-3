...
def handle_timer():
    global show_dismissed_message, exit_camera, start_time, countdown_count, warning_display_time, warning_active
    while True:
        if start_time > 0 and not warning_active:
            ...
            if elapsed_time >= 5:
                ...
                if countdown_count < 3:
                    warning_active = True
                    warning_display_time = time.time()
                else:
...
while True:
    ...
    if warning_active:
        if time.time() - warning_display_time < 3:  # Display warning for 3 seconds
            processed_frame[:] = (0, 0, 255)  # Make the frame red
